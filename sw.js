const CACHE = 'faceup-v1.130.0';
const APP_ROOT = new URL('./', self.registration.scope).href;
const INDEX_URL = new URL('./index.html', self.registration.scope).href;
const APP_FILES = [APP_ROOT, INDEX_URL, new URL('./styles.css?v=1.130.0', self.registration.scope).href, new URL('./app.js?v=1.130.0', self.registration.scope).href, new URL('./manifest.webmanifest?v=1.130.0', self.registration.scope).href, new URL('./icon.svg', self.registration.scope).href, new URL('./icon-maskable.svg', self.registration.scope).href];
